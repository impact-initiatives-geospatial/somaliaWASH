...
map_with_tmap <- function(input_shepefile, input_points, input_col) {
  tm_shape(input_shepefile) +
    ...
    tm_symbols(col = input_col, size = 0.3, palette = "Accent", border.lwd = 0.1)+
    tm_compass(type = "4star", size = 2, position = c("left", "top"))+
    tm_scale_bar(position = c("right", "bottom"))
}

map_with_ggplot <- function(input_shepefile, input_points, input_col) {
  ggplot()+
  geom_sf(data = input_shepefile, 
               fill = "lightgray",
               colour = "white") + 
  geom_point(data = input_points, aes(x=Longitude, y =Latitude, colour = input_col), size = 1.5) +
  geom_jitter() +
...
purrr::map(analysis_cols_shocks, ~map_with_tmap(df_som_data_shape, df_som_data_sf, .x))
