...
#'
#' @examples
map_with_tmap <- function(input_shepefile, input_points, input_col) {
  tm_shape(input_shepefile) +
    tm_polygons() +
    tm_layout(paste("Map of SOM showing", input_col), title.size=.8)+
    tm_shape(input_points)+
    tm_symbols(col = input_col, size = 0.5, palette = "Accent")+
...
maps_shocks <- purrr::map(analysis_cols_shocks, ~map_with_tmap(df_som_data_shape, df_som_data_sf, .x))

cowplot::plot_grid(plotlist = maps_shocks)
...
maps_displacement <- purrr::map(analysis_cols_displacement, ~map_with_tmap(df_som_data_shape, df_som_data_sf, .x))
cowplot::plot_grid(plotlist = maps_displacement)
```
...
maps_wash <- purrr::map(analysis_cols_wash, ~map_with_tmap(df_som_data_shape, df_som_data_sf, .x))
cowplot::plot_grid(plotlist = maps_wash)
```
